...
def main():
    ...
    logger.info("="*80)
    logger.info("GONZO BOT - Phase 0 MVP")
    logger.info("="*80)
    ...
    try:
        ...
        # PHASE 0: For the MVP, create a simple test conversation plan
        # In Phase 1+, this will be generated by Claude API based on scan results
        # Question must align with what Demeter AI can actually answer (see coverage_manifest.yaml)
        logger.info("\n[PHASE 0 TEST] Creating test conversation plan...")
        test_conversation_plan = [{
            "topic": "California almond growing conditions and agronomic trends",
            "opening_question": "What are the current growing conditions for California almonds based on the DASI index, and how do they compare to the 10-year average?"
        }]
        logger.info(f"Created {len(test_conversation_plan)} test conversation(s)")

        # PHASE 2: Interrogate Demeter AI
        logger.info("\n[PHASE 2] Interrogating Demeter AI...")
        transcripts = interrogate(config, test_conversation_plan)
        ...
        posts = generate(config, transcripts, run_id)
        ...
        log_file = log_run(run_id, config, scan_results, transcripts, posts, publishing_summary)
